...
formatBroadcastMessage(message, type = 'text') {
  ...
  const style = styles[type] || styles.text; // Fallback to text style
  
  const styledMessage = `${style.icon} *${style.title}* ${style.icon}\n\n${message}`;

  return styledMessage + footer;
}

// Get thread options for message sending
getThreadOptions(update) {
...
formatMediaCaption(caption, mediaType) {
  ...
  const style = styles[mediaType] || styles.photo; // Fallback to photo style
  const timestamp = new Date().toLocaleDateString('en-US', {
    weekday: 'long',
    year: 'numeric',
    month: 'long',
...
formatErrorMessage(message) {
  ...
  const errorType = typeof message === 'string' && errorMessages[message] ? errorMessages[message] : message;
  
  return `❌ *ERROR*\n\n${errorType}\n\n🔧 Please try again or contact the admin if the problem persists.`;
}

// Format success message
formatSuccessMessage(message, type = 'general') {
  const successStyles = {
    general: { icon: "✅", title: "SUCCESS" },
    created: { icon: "📝", title: "CREATED" },
    updated: { icon: "🔄", title: "UPDATED" },
    deleted: { icon: "🗑️", title: "DELETED" },
    sent: { icon: "📤", title: "SENT" }
  };
  
  const style = successStyles[type] || successStyles.general;
...
formatInfoMessage(message) {
  return `ℹ️ *INFORMATION*\n\n${message}\n\n💡 For more details, contact the admin.`;
}
...
┅┅┅┅┅┅┅┅┅┅┅┅
👥 **Total:** ${currentGroupMembers} Active Users
...
if (text.startsWith("/userlist")) {
    // Determine the message ID to reply to, prioritizing the menu message ID
    const targetMessageId = menuMessageIds.get(chatId) || messageId;
    let messageIdToDelete;

    // 1. Send the loading message
    const loadingMessage = await this.sendMessage(
        ...
        "```Running\nPlease wait while it is being processed...```",
        {
            parse_mode: "Markdown",
            reply_to_message_id: targetMessageId,
            ...options // Assuming 'options' holds common parameters like disable_web_page_preview
        }
    );

    // Safely get the ID of the loading message if the send operation was successful
    messageIdToDelete = loadingMessage?.result?.message_id;

    try {
        // 2. Generate and send the user list page
        ...
        // Handle potential errors here, e.g., logging or sending a failure message
        console.error("Error generating or sending user list:", error);
    } finally {
        // 3. Delete the loading message regardless of success or failure
...
 *Kirim link Anda sekarang!*`,
    ...
    if (text.startsWith("/randomconfig")) {
        ...
        {
            parse_mode: "Markdown", 
reply_to_message_id: targetMessageId, ...options });
        let messageIdToDelete;
...
 *Terima kasih telah menggunakan layanan kami!*`,
          ...
          reply_markup: {
            inline_keyboard: [
              ...
              [
                { text: "🌐 Web VPN", url: "https://joss.jak.biz.id" },
...
async function randomconfig() {
  try {
    const HOSTKU2 = "joss.jak.biz.id";
    const GITHUB_BASE_URL = "https://raw.githubusercontent.com/jaka1m/botak/main/cek/";
    ...
    // Check proxy status
    const checkResponse = await fetch(`https://api-check.web.id/check?ip=${ip}:${port}`);
    if (!checkResponse.ok) {
...
try {
    ...
    const statusResponse = await fetch(`https://api-check.web.id/check?ip=${ip}:${port}`);
    ...
    // PERBAIKAN: Memperbaiki sintaks if yang salah
    ...
    const HOSTKU = "joss.jak.biz.id";
    const path = `/Free-VPN-CF-Geo-Project/${countryCode.toUpperCase()}`;
    const flagEmoji = getFlagEmoji(countryCode);
    
    // PERBAIKAN: Menambahkan pengecekan untuk menghindari undefined
    const isp = ipData.isp || provider || 'Unknown ISP';
    const countryName = ipData.country || country || countryCode.toUpperCase();
    
    const encodedVlessLabelTLS = encodeURIComponent(`ROTATE VLESS ${isp} ${countryName} TLS`);
    const encodedVlessLabelNTLS = encodeURIComponent(`ROTATE VLESS ${isp} ${countryName} NTLS`);
    const encodedTrojanLabelTLS = encodeURIComponent(`ROTATE TROJAN ${isp} ${countryName} TLS`);
    const encodedSsLabelTLS = encodeURIComponent(`ROTATE SHADOWSOCKS ${isp} ${countryName} TLS`);
    const encodedSsLabelNTLS = encodeURIComponent(`ROTATE SHADOWSOCKS ${isp} ${countryName} NTLS`);
...
${generateVMessConfigString(ip, port, `ROTATE VMESS ${isp} ${countryName}`)}
...
if (navButtons.length) inline_keyboard.push(navButtons);
  ...
  inline_keyboard.push([
    ...
    { text: "💰 Donate", callback_data: "menu_cmd_donate" }
]);
...
...options
  ...
  const targetMessageId = menuMessageIds.get(chatId) || messageId;
  const CLOUDFLARE_API_TOKEN = "h1tbNK-lWqLXXPGSSHNKlQcjR7p9xDqVBdc62PCm";
  const CLOUDFLARE_ZONE_ID = "5c7647b716ed5262595513d4becaf9ed";
...
const DEFAULT_HOST = "joss.jak.biz.id";
const API_URL = "https://api-check.web.id/check?ip=";
...
function generateVMessConfigString(ip, port, isp) {
  const pathh = `/Free-VPN-CF-Geo-Project/${ip}=${port}`;
  const uuid1 = 'f282b878-8711-45a1-8c69-5564172123c1';
  const DEFAULT_HOST = "vmess.jak.biz.id";
  const vmessTLS = {
    v: "2",
    ps: `[VMess-TLS] ${isp}`,
    add: DEFAULT_HOST,
...
function generateConfig(config, protocol, wildcardKey = null) {
  ...
  if (protocol === "VMESS") {
    const vmessConfig = generateVMessConfigString(config.ip, config.port, config.isp);
    const vmessTLSLine = vmessConfig.split('\n')[2];
    qrUrl = `https://api.qrserver.com/v1/create-qr-code/?data=${encodeURIComponent(vmessTLSLine)}&size=400x400`;
    configOutput = `
${vmessConfig}
...
const APIKU = "https://api-check.web.id/check?ip=";
const DEFAULT_HOST2 = "joss.jak.biz.id";
...
\`\`\``;
        ...
        configText = generateVMessConfigString(ip, port, prov);
      } else if (type === "trojan") {
...
const TelegramBot = class {
  ...
  async handleUpdate(update) {
    ...
    try {
        await this.sendMessage(chatId, firstMessage, { 
            reply_to_message_id: targetMessageId, 
            ...options 
        });
...
async function getCountryKeyboard(page = 0) {
    ...
    const navigationRow = [];
    if (page > 0) {
        navigationRow.push({ text: "◀️ Prev", callback_data: `sublink_page_${page - 1}` });
    }
    navigationRow.push({ text: `Page ${page + 1}/${totalPages}`, callback_data: "sublink_ignore" }); 
    if (page < totalPages - 1) {
        navigationRow.push({ text: "Next ▶️", callback_data: `sublink_page_${page + 1}` });
    }
  ...
  if (update.callback_query) {
   ...
   if (step === "app" && state.step === "app") {
    ...
    try {
     ...
     const caption = `<b>🔗 Sublink Created Successfully!
...
${url}</b>`;
     ...
     await this.deleteMessage(chatId, state.processingMessageId);
     await this.sendMessage(chatId, `<b>❌ Terjadi Kesalahan

${error.message}

Silakan coba lagi dengan parameter yang berbeda.</b>`, {
      parse_mode: "HTML",
            reply_to_message_id: state.targetMessageId,
      ...options
     });
...
const worker_default = {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      const token = "7664381872:AAHk05xKJ-ibwHOKPMe7prr9Vx0R35jHzYA";
      ...
      const apiKey = "fa084f134063e11cd26e93fbbe7e1f468dc91";
      const accountID = "fbfeda4c47ab2397865928d1766f6415";
      const zoneID = "91920c3ea37495f5de7e15d31be40038";
      const apiEmail = "desalekong24@gmail.com";
      const serviceName = "joss";
      const rootDomain = "jak.biz.id";
